...
class genericPlant(object):
	...
	def growPlant(self, theGarden):
		...
		if self.makeSeeds:
			...
			if (self.heightStem>=self.startMakingSeedsHeight) or (self.age>=self.startMakingSeedsAge) or self.isMature:
				...
				###I'm using area here vs area of photosynthesis because for single, unshaded plants, this is the same
				#areaLeaf=3.14*self.radiusLeaf*self.radiusLeaf
				#if self.leafIsHemisphere:
				#	areaLeaf=areaLeaf*2.0
				#maxKgSeeds=self.reproductionConstant*math.pow(areaLeaf, self.reproductionExponent)
				#currentKgMatureSeeds=numbSeeds*self.massSeedMax
				#while (numbSeeds*self.massSeedMax) < maxKgSeeds:
				#	self.makeSeed(self)
				#	numbSeeds=float(len(self.seedList))
		####
		##let yourself grow
	...
	def makeSeed(self, theSeed):
		...
		if self.leafIsHemisphere:
			...
			if dist==self.radiusLeaf:
				...
				deltaY=self.radiusLeaf-math.sqrt(abs(math.pow(self.radiusLeaf,2.0)-math.pow(dist,2.0)))
	...
	def growSeedOnPlant(self, maxCarbonForSeed):
		...
		if self.motherPlant.leafIsHemisphere:
			...
			if dist==self.motherPlant.radiusLeaf:
				...
				deltaY=self.motherPlant.radiusLeaf-math.sqrt(abs(math.pow(self.motherPlant.radiusLeaf,2.0)-math.pow(dist,2.0)))
	...
	def disperseSeed(self, theSeed, motherPlant, theGarden):
		...
		if motherPlant.seedDispersalMethod[0]==0:
			...
			tempVar3= math.sqrt(math.pow(tempVar2, 2.0)+(2*g*h))
	...
	def germinate(self, theGarden):
		if self.countToGerm<1:
			...
			if tooBad<self.fractionFailGerminate:
				self.causeOfDeath="failed to germinate(other)"
				theGarden.kill(self)
			else:
	...
	def calcNewMassFromLeaf(self, theGarden):
		...
		if fractionAvailable>self.fractionMinimumSurvival:
			lightConversion=self.photoConstant*(math.pow(self.massLeaf, self.photoExponent))#in units of kgGrowth/area leaf for photosynthesis
	...
	def calcMassLeafFromMassStem(self):
		...
		#	print "Age: %i  expected Ml: %f  actual Ml: %f" % (self.age, massTotal-massStem, massLeaf)
	...
	def calcRadiusLeafFromMassLeaf(self):
		...
		#####used in trying to get seed later on
		self.leafAreaDisc=leafAreaDisc
		######
	...
	def calcHeightStemFromRadiusStem(self, theGarden):
		...
		###experiment with deriving alpha###
		#var0=self.heightStem+self.speciesConstant6
		#if var0>0.0:
		#	var1=math.log(var0)
		#	var2=math.log(self.speciesConstant7)
		#	var3=math.log(Ds)
		#	calc1=(var1-var2)/var3
		#
		#	print "cycle: %i   calculation: %f   exponent: %f" % (theGarden.cycleNumber, calc1, self.speciesExponent7)
		...
		if Hs<self.heightStem:
			...
			self.isMature=True
			#print "*****cycle: %i   plant is mature*****" % (theGarden.cycleNumber)
			#Hs=(self.speciesConstant7*math.pow(Ds, self.speciesExponent7))-self.speciesConstant6
			###this is used the calculate a constant to pin the max number of seeds at 10.
			####Used later when actually making seeds
			if self.seedsPerAreaConstant==None or self.seedsPerAreaConstant=="NULL":
				maxSeedsPerPlant=int(theGarden.maxSeedsPerPlant)
				maxKgSeeds=self.reproductionConstant*math.pow(self.massFixed, self.reproductionExponent)
				self.seedsPerAreaConstant=maxKgSeeds/(maxSeedsPerPlant*math.pow(maxKgSeeds, self.seedsPerAreaExponent))
