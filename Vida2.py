...
def saveDataPoint (theDirectory, theFileName, theGarden):
	...
	for plant in theGarden.deathNote:
		if plant.age>0:
			...
			theData="%i,%s,%s,%s,%f,%f,%s,%s,%i,%i,%f,%f,%i,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%s \n" % (theGarden.cycleNumber,plant.name,plant.nameSpecies,plant.motherPlant,plant.x,plant.y,plant.isSeed,plant.isMature,plant.countToGerm,plant.age,plant.massStem,plant.massLeaf,len(plant.seedList),plant.massSeedsTotal,plant.massStem+plant.massLeaf,plant.massTotal,plant.radiusStem*2,plant.radiusLeaf,plant.areaCovered,plant.heightStem,plant.heightLeafMax,plant.z,plant.GMs,plant.GMl,plant.GMs+plant.GMl,2.0*plant.GRs,plant.GHs,0,0,0,0,0,plant.causeOfDeath)
...
def main():
	...
	for x in range(timesToRepeat):
		...
		if produceGraphics and CFDGtext=="":
			###this should eventually be a loop for different views
#CFDGtext=outputGraphics.initCFDGText(theGarden, int(graphicalView[0]), percentTimeStamp, 50.0)	
			CFDGtext=outputGraphics.initCFDGText(theGarden, 1, percentTimeStamp, 50.0)
		#######
		cycleNumber=0
		print "\n***Running simulation.***"
		...
		while (theGarden.numbPlants<=maxPopulation and cycleNumber<=maxCycles) and (theGarden.numbPlants+theGarden.numbSeeds)>0:
			...
			if produceGraphics==True:
				...
				if "3d"!=graphicalView[0]:
					theData=outputGraphics.makeCFDG(graphicalView[0], CFDGtext, theGarden, cycleNumber)
                #if webOutput==False:
					if theView==1:
...
#				saveSimulationPoint(saveDirectory, fileName, theGarden)
		...
		if produceGraphics==True and deleteCfdgFiles==True:
...
if __name__ == '__main__':
    ...
    #there will have to be more massaging of things before going on to main()
    main()
else:
    main()
